 ...
 *
 * Spring Security 없이 순수 세션 기반 인증을 WebSocket에 연동할 때 사용한다.
 * SessionHandshakeInterceptor가 먼저 실행되어 attributes에 member_id를 저장하고,
 * 이 클래스가 그 값을 꺼내 Principal 객체로 만든다.
 *
 * 생성된 Principal은 WebSocket 연결 동안 유지되며,
 * STOMP 컨트롤러 메서드에서 Principal 파라미터로 자동 주입된다.
 */
@Component
public class MemberPrincipalHandshakeHandler extends DefaultHandshakeHandler {

    /**
     * WebSocket 세션에 연결될 Principal을 결정한다.
     * SessionHandshakeInterceptor → 이 메서드 순서로 실행된다.
     *
     * @param attributes SessionHandshakeInterceptor에서 저장한 값이 들어있다
     */
    @Override
    protected Principal determineUser(ServerHttpRequest request, WebSocketHandler wsHandler,
        ...
        // principal.getName() 호출 시 memberId(UUID)가 반환된다
        return () -> memberId;
    }
}
